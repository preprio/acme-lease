            ...
            <nav aria-label="Global">
                <Container className="flex items-center justify-between">
                    <Link href="/" className="hover:cursor-pointer">
                        <Image src="/logo.svg" width={150} height={40} alt="Demo Logo" className="max-h-10" />
                    </Link>
                    ...
                    <div className="hidden lg:flex items-center gap-8">
                        ...
                        <div className="flex items-center gap-12">
                            {props.items.map((item, index) => (
                                <PreprButton button={item} key={index} />
                            ))}
                        </div>
                    </div>

                    {/* Mobile menu */}
                    <Dialog open={mobileMenuOpen} onClose={setMobileMenuOpen} className="lg:hidden z-30" as="div">
                        <div className="fixed inset-0 z-10" />
                        <DialogPanel
                            className="fixed inset-y-0 bg-primary-100 right-0 z-10 w-full overflow-y-auto sm:max-w-sm">
                            <div className="flex py-4 p-spacing items-center justify-between sm:justify-end">
                                <Link href="/" className="-m-1.5 p-1.5 sm:hidden hover:cursor-pointer">
                            ...
                            <div className="py-10 px-4 items-center justify-center space-y-12 flex flex-col">
                                {props.items.map((item, index) => (
                                    <div onClick={handleMobileMenuClose} key={index}><PreprButton button={item}
